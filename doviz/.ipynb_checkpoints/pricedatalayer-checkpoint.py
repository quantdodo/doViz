...
class priceLocal:
    def __init__(self,symbol):
        self.callno = 0
        #self.pricedata = pd.read_pickle("pricevectors.pkl")
        self.cryptodata = pd.read_csv("cryptodata/cryptomarkets.csv")
        self.cryptodata[self.cryptodata['symbol'] == symbol]
        self.newdata = self.cryptodata['close']
        
    def getPrice(self):
        price = self.newdata.loc[self.callno]
        self.callno+=1
        return price
...
class tickerPriceLayer:
    def __init__(self, tikr,simno):
        self.ticker = tikr
        self.eblogo = Image(filename='D:/workspaced/plotstuff/realtime-graph/logos/dovizdark.png')
...
class priceDataLayer:
    ...
    def simulate(self,typex="priceonly",plotsx=None):
